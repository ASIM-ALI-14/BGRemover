...
class VideoFrameDecoder(
    ...
    fun probeVideoInfo(): VideoInfo? {
        // Rotation is only in MediaMetadataRetriever, not MediaExtractor
        val rotation = try {
    ...
    suspend fun decode(onFrame: suspend (Bitmap, Long, Float) -> Boolean) {
        ...
        val (trackIdx, fmt) = findVideoTrack(ext)
        ...
        ext.release()
        codec = null; extractor = null
    }

    fun release() { codec?.stop(); codec?.release(); extractor?.release() }
...
// Copies byte planes to arrays first (eliminates per-byte JNI cost)
