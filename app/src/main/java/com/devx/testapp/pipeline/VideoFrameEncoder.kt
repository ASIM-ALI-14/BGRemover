package com.devx.testapp.pipeline
...
class VideoFrameEncoder(
    ...
    // Auto bitrate: 0.2 bits per pixel per frame, clamped 4–20 Mbps
    ...
    // NDC quad + flipped UV (bitmap top → screen top)
    ...
    fun start() {
        ...
        Log.i(TAG, "Encoder: ${width}×${height} @${fps}fps ${bitrate/1000}kbps")
    }

    fun encodeFrame(bitmap: Bitmap, ptsUs: Long) {
        // Upload bitmap → GL texture
        ...
        fun floatBuf(arr: FloatArray) = ByteBuffer
            .allocateDirect(arr.size * 4).order(ByteOrder.nativeOrder())
            .asFloatBuffer().also { it.put(arr); it.position(0) }

        GLES20.glEnableVertexAttribArray(aPos)
        GLES20.glVertexAttribPointer(aPos, 2, GLES20.GL_FLOAT, false, 0, floatBuf(verts))
        GLES20.glEnableVertexAttribArray(aUV)
        GLES20.glVertexAttribPointer(aUV,  2, GLES20.GL_FLOAT, false, 0, floatBuf(uvs))
    ...
    fun finish() {
        ...
        codec = null; muxer = null; encoderSurface = null; muxerStarted = false
    }
    ...
    private fun initEgl() {
        ...
        val att = intArrayOf(
            ...
            EGL_RECORDABLE, 1, EGL14.EGL_NONE
        )
        val cfgs = arrayOfNulls<EGLConfig>(1); val n = IntArray(1)
        EGL14.eglChooseConfig(eglDisplay, att, 0, cfgs, 0, 1, n, 0)
        check(n[0] > 0) { "No EGL config" }

        eglContext = EGL14.eglCreateContext(eglDisplay, cfgs[0]!!,
            EGL14.EGL_NO_CONTEXT,
            intArrayOf(EGL14.EGL_CONTEXT_CLIENT_VERSION, 2, EGL14.EGL_NONE), 0)
        eglSurface = EGL14.eglCreateWindowSurface(eglDisplay, cfgs[0]!!,
            encoderSurface, intArrayOf(EGL14.EGL_NONE), 0)
    ...
    private fun drain(eos: Boolean) {
        ...
        while (true) {
            ...
            when {
                ...
                idx == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> if (!muxerStarted) {
