...
object AudioMuxer {
    ...
    private const val BUFFER_SIZE = 2 * 1024 * 1024 // 2 MB read buffer
    ...
    fun mux(
        ...
        while (true) {
            ...
            info.flags              = videoExt.sampleFlags  // read actual sync flags per sample
            info.offset             = 0
            muxer.writeSampleData(muxVideo, buf, info)
            videoExt.advance()
        }

        // 5. Write audio — seek to trim start, normalize PTS to 0
        ...
        while (true) {
            ...
            // Offset audio PTS to match video (both start from 0)
            info.presentationTimeUs = (audioExt.sampleTime - trimStartUs).coerceAtLeast(0L)
            info.flags              = audioExt.sampleFlags  // read actual sync flags per sample
            info.offset             = 0
