...
class VideoProcessingPipeline(private val context: Context) {
    ...
    suspend fun process(
        ...
        val effectiveTrimEnd = if (trimEndUs == Long.MAX_VALUE) info.durationUs else trimEndUs
        val trimDuration     = (effectiveTrimEnd - trimStartUs).coerceAtLeast(1L)

        Log.i(TAG, "Input: ${info.width}×${info.height} rot=${info.rotation}° @${info.fps}fps")
        Log.i(TAG, "Trim: ${trimStartUs / 1_000_000}s → ${effectiveTrimEnd / 1_000_000}s")

        // 2. Compute encoder dimensions (swap w/h for rotated video)
        val (encW, encH) = when (info.rotation) {
            90, 270 -> info.height.align() to info.width.align()
            else    -> info.width.align()  to info.height.align()
        }

        // 3. MP4 cannot store alpha — fall back to black background
        val effectiveBg = if (background is FrameMaskApplier.BackgroundMode.Transparent)
            FrameMaskApplier.BackgroundMode.SolidColor(Color.BLACK)
        else background

        // 4. Set up encoder (writes to temp file; audio muxed later)
        ...
        try {
            ...
            decoder.decode { raw, ptsUs, _ ->
                ...
                withContext(Dispatchers.Main) {
                    onProgress(progress * 0.90f, if (isPreview) processed else null)
                }
            ...
            // Always clean up even if decode throws or coroutine is cancelled
            encoder.finish()
            segmenter.close()
            previewBitmap?.recycle()
            previewBitmap = null
    ...
    // Rotate bitmap if video has a rotation flag
    ...
    // Align to nearest even number (required by H.264 encoder)
